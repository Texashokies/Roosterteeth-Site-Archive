...
class Archive {
  constructor({ name, url = '', description = '', deepLink='',deepLinkAddition='',downloadLink='',downloadSize='',tag='' }) {
    this.url = url;
...
function makeArchive(data, idx) {
  let name, url,deepLink,deepLinkAddition,downloadLink,downloadSize,tag;
  ...
  if (url) {
    return new Archive({ name, url, description: data.description, deepLink,deepLinkAddition, downloadLink,downloadSize,tag});
  }
